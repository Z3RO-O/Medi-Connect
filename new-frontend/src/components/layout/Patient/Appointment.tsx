...
const Appointment = () => {
    ...
    const { doctors, currencySymbol, backendUrl, token, getDoctosData } = useContext(AppContext) as IPatientAppContext
    ...
    const [vitals, setVitals] = useState<{ bpm: string | number; spo2: string | number }>({ bpm: '---', spo2: '---' })
    ...
    const bookAppointment = async () => {
        ...
        const slotDate = day + "_" + month + "_" + year
        try {
            const { data } = await axios.post(backendUrl + '/api/user/book-appointment', { docId, slotDate, slotTime, vitals }, { headers: { token } })
            if (data.success) {
        ...
        <div>
            ...
            <div className='sm:ml-72 sm:pl-4 mt-8 font-medium text-[#565656]'>
                ...
                <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
                    {docSlots.length > 0 && docSlots.map((item: Slot[], index: number) => (
                ...
                <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
                    {docSlots.length > 0 && docSlots[slotIndex].map((item: Slot, index: number) => (
